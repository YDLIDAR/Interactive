...
CYdLidar::CYdLidar(): lidarPtr(0) {
  ...
  m_SampleRate = 9;
  m_ScanFrequency = 7;
  m_DeviceType    = LIDAR_DEVICE_DRIVER_TYPE_SERIALPORT;
  isScanning = false;
  node_counts = 720;
  each_angle = 0.5;
  show_error = 0;
...
bool  CYdLidar::checkCOMMs() {
  ...
  // Is it COMX, X>4? ->  "\\.\COMX"
  if (m_DeviceType == LIDAR_DEVICE_DRIVER_TYPE_SERIALPORT && m_SerialPort.size() >= 3) {
  ...
  if (op_result != RESULT_OK) {
    if (m_DeviceType == LIDAR_DEVICE_DRIVER_TYPE_SERIALPORT) {
...
again:
  ...
  if (!ret || !getDeviceInfo(m_type)) {
    if (m_DeviceType == LIDAR_DEVICE_DRIVER_TYPE_TCP)
      return false;
  ...
  m_Intensities = false;
  if (m_type == 4) {
    if (m_SerialBaudrate == 153600 && m_DeviceType == LIDAR_DEVICE_DRIVER_TYPE_SERIALPORT)
      m_Intensities = true;
    if (m_Intensities) {
