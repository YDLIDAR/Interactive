...
result_t YDlidarDriver::connect(const char* port_path, uint32_t baudrate) {
  ...
  if (m_driver_type != LIDAR_DEVICE_DRIVER_TYPE_SERIALPORT && m_driver_type != LIDAR_DEVICE_DRIVER_TYPE_TCP) {
    m_driver_type = LIDAR_DEVICE_DRIVER_TYPE_SERIALPORT;
  }
  {
    ...
    if ((ans = waitScanData(local_buf, count)) != RESULT_OK) {
      if (ans != RESULT_TIMEOUT || timeout_count > DEFAULT_TIMEOUT_COUNT) {
        if (!isAutoReconnect) { //不重新连接, 退出线程
          ...
          fprintf(stderr, "[%llu]: start reconnecting in  %s \n", time, serial_port.c_str());
          fflush(stderr);
          while (isAutoReconnect && isAutoconnting) {
            ...
            while (isAutoReconnect && connect(serial_port.c_str(), _baudrate) != RESULT_OK) {
              delay(500);
              time = getTime();
              fprintf(stderr, "[%llu]: wait %s is available\n", time, serial_port.c_str());
              fflush(stderr);
            }
            if (!isAutoReconnect) {
              isScanning = false;
            ...
            if (isconnected()) {
              ...
              if (!ret) {
                time = getTime();
                fprintf(stderr, "[%llu]:Failed to check heartbeat\n", time);
                fflush(stderr);
                continue;
              ...
              if (ans == RESULT_OK) {
                ...
                time = getTime();
                fprintf(stderr, "[%llu]:Failed to start lidar scan\n", time);
                fflush(stderr);
                delay(1000);
              }
...
result_t YDlidarDriver::waitPackage(node_info* node, uint32_t timeout) {
  ...
  if (package_Sample_Index == 0) {
    uint8_t recvBuffer[MAX_RECV_SIZE];
    recvPos = 0;
    while ((waitTime = getms() - startTs) <= timeout) {
  ...
  if ((*node).sync_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT) {
    ...
    uint64_t time_diff = m_ns - m_last_ns;
    if (time_diff < 0) {
      m_ns = m_last_ns;
...
result_t YDlidarDriver::setScanFrequency(scan_frequency& frequency, uint32_t timeout) {
  result_t  ans;    scan_frequency CurrentFreq;
  if (!isConnected) {
    return RESULT_FAIL;
  }
  disableDataGrabbing();
  if ((ans = getScanFrequency(CurrentFreq, timeout) != RESULT_OK)) {
    return ans;
  }
  if (frequency.frequency > 1250)
    frequency.frequency = 1250;

  int32_t FreqDiff = frequency.frequency - CurrentFreq.frequency;
  int32_t OP_Mac = FreqDiff / 100;
  int32_t OP_Mic = (FreqDiff - OP_Mac * 100) / 10;
  while (OP_Mac > 0) {
    if ((ans = setScanFrequencyAdd(CurrentFreq, timeout) != RESULT_OK)) {
      return ans;
    }
    OP_Mac--;
  }   while (OP_Mac < 0) {
    if ((ans = setScanFrequencyDis(CurrentFreq, timeout) != RESULT_OK)) {
      return ans;
    }
    OP_Mac++;
  }   while (OP_Mic > 0) {
    if ((ans = setScanFrequencyAddMic(CurrentFreq, timeout) != RESULT_OK)) {
      return ans;
    }
    OP_Mic--;
  }   while (OP_Mic < 0) {
    if ((ans = setScanFrequencyDisMic(CurrentFreq, timeout) != RESULT_OK)) {
      return ans;
    }
    OP_Mic++;
  }
...
/**
 * @brief YDlidarDriver::checkTransTime
 */
